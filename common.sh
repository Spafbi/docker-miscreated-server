...
if [[ "$(basename ${0})" == "common.sh" ]]; then
	echo Do not run this file directly. Please use one of the "run" commands.
	exit 1
fi
...
runString="Bin64_dedicated/MiscreatedServer.exe -sv_port=${sv_port} -mis_gameserverid=${mis_gameserverid} +sv_maxplayers ${maxplayers} +http_startserver +map ${map}${whitelisted}"
